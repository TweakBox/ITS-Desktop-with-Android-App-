...
public class Database
{
    ...
    public bool InsertRecord(string table, Dictionary<string, dynamic> mappedValues)
    {
        string command = "insert into " + table + "(`" + String.Join("`,`", mappedValues.Keys) + "`) values(@" + String.Join(",@", mappedValues.Keys) + ")";

        Command.Parameters.Clear();
        foreach (var map in mappedValues)
        {
            if(map.Value.GetType() == typeof(byte[]))
            {
                Command.Parameters.Add("@" + map.Key, MySqlDbType.LongBlob).Value = map.Value;
            }
            else
            {
                Command.Parameters.Add("@" + map.Key, map.Value);
            }
        }
    ...
    public bool UpdateRecord(string table, string findKey, string findValue, Dictionary<string, dynamic> mappedValues)
    {
        List<string> updateColumnString = new List<string>();
        foreach(var map in mappedValues)
        {
            updateColumnString.Add(map.Key + "=@" + map.Key);
        }

        string command = "update " + table + " set " + String.Join(",", updateColumnString) + " where " + findKey + "='" + findValue + "'";
